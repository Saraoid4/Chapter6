...
try:
    from injectfrb import reader, simulate_frb, tools, rfi
except ImportError:
    import reader
    import simulate_frb
    import tools
...
def inject_in_filterbank_gaussian(
    data_fil_obj, header, fn_fil_out, N_FRB, chunksize=2**11
):
    NFREQ = header["nchans"]
    ...
    for ii in range(N_FRB):
        if ii == 0:
            reader.write_to_fil(np.zeros([NFREQ, 0]), header, fn_fil_out)
        ...
        if ii < 0:
            print("This branch is currently unused")  # Remove undefined data_chunk reference
        elif ii >= 0:
...
def inject_in_filterbank(
    ...
    data_fil_obj_skel, freq_arr, dt, header = reader.read_fil_data(
        fn_fil, start=0, stop=1
    )
    NFREQ = header["nchans"]
    ...
    for ii in range(N_FRB):
        if params_arr is not None:
            ...
            # disp_ind unused variable, commenting out
        ...
        #            fluence *= ((width_sec**2 + dt**2 + tdm**2)**0.5 / width_sec)
            ...
            fluence = np.random.uniform(0, 1) ** (-2 / 3.0)
            dm = np.random.uniform(10.0, 2000.0)
            scat_tau_ref = 0.0
            spec_ind = 0.0
            width_sec = 2 * dt
        ...
        if gaussian_noise is True:
            ...
            t_delay_max_pix = int(3 * t_delay_max / dt)
            t_chunksize_min = 3.0
            chunksize = 2 ** (np.ceil(np.log2(t_delay_max_pix)))
            chunksize = int(max(t_chunksize_min / dt, chunksize))
        ...
        if gaussian_noise is True:
            if simulator == "injectfrb":
                ...
                noise_event = np.random.normal(128, noise_std, NFREQ * NTIME).reshape(
                    NFREQ, NTIME
                )
            ...
            NTIME = int(2 * t_delay_max / dt)
            if data.shape[1] < NTIME:
                print("Not enough data in the filterbank file. Not injecting.")
                return
            data_event = (data[:, offset : offset + NTIME]).astype(np.float64)
        ...
        if simulator == "injectfrb":
            ...
            data_event = (
                data_event.reshape(NFREQ, upchan_factor, NTIME, upsamp_factor)
                .mean(-1)
                .mean(1)
            )
            data_event *= 20.0 * noise_std / np.sqrt(NFREQ)
            ...
            data_event *= 10.0 * noise_std / np.sqrt(NFREQ)

            # [dm, fluence, width, spec_ind, disp_ind, scat_tau_ref]
            params = [dm, fluence, width_sec, spec_ind, 2.0, scat_tau_ref]
        ...
        print("CHECK WHERE DATA ", data.shape)
        ...
        widths_snr = range(int(max(downsamp / 2.0, 1)), int(min(downsamp * 2, 2500)))
        ...
        print(
            "S/N: %.2f width_used: %.1f width_tru: %.1f DM: %.1f"
        ...
        if rfi_clean is True:
            data = rfi.apply_rfi_filters(data.astype(np.float32), dt)
        ...
        if clipping is not None:
            ...
            stds, med = SNRTools.sigma_from_mad(data_ts_zerodm)
            ind = np.where(np.absolute(data_ts_zerodm - med) > 8.0 * stds)[0]
            data[:, ind] = np.median(data, axis=-1, keepdims=True)

        if ii < 0:
            reader.write_to_fil(data.transpose(), header, fn_fil_out)
        elif ii >= 0:
            fil_obj = reader.filterbank.FilterbankFile(fn_fil_out, mode="readwrite")
            fil_obj.append_spectra(data.transpose())
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        dest="sig_thresh",
        type="float",
        help="Only process events above >sig_thresh S/N" "(Default: 8.0)",
        default=8.0,
    )
    ...
    parser.add_option(
        "--rfi_clean", dest="rfi_clean", default=False, help="apply rfi filters"
    )
    ...
    parser.add_option(
        "--dm_list", type="string", action="callback", callback=foo_callback
    )
    ...
    parser.add_option(
        ...
        dest="gaussian",
        action="store_true",
        help="write only Gaussian data to fil files",
        default=False,
    )
    ...
    parser.add_option(
        ...
        dest="gaussian_noise",
        action="store_true",
        help="use Gaussian background noise",
        default=False,
    )
    ...
    if len(options.dm_list) == 1:
        inject_in_filterbank(
            fn_fil,
            fn_fil_out,
            N_FRB=options.nfrb,
            NFREQ=options.nfreq,
            NTIME=2**15,
            rfi_clean=options.rfi_clean,
            calc_snr=bool(options.calc_snr),
            start=0,
