...
class Event(object):
    ...
    def __init__(
        self, t_ref, f_ref, dm, fluence, width, spec_ind, disp_ind=2, scat_tau_ref=0
    ):
        self._t_ref = t_ref
        self._f_ref = f_ref
        self._dm = dm
    ...
    def disp_delay(self, f, _dm, _disp_ind=-2.0, k_DM=1e3 / 0.241):
    ...
    def calc_width(
        self, dm, freq_c, bw=400.0, NFREQ=1024, ti=0.001, tsamp=0.001, tau=0.0
    ):
        ...
        tI = np.sqrt(ti**2 + tsamp**2 // 10 + tdm**2 + tau**2)
        return 1e-3 * tI
    ...
    def scintillation(self, freq):
        ...
        envelope = np.cos(
            2 * np.pi * nscint * freq**-2 / self._f_ref**-2 + scint_phi
        )
        envelope[envelope < 0] = 0
        return envelope

    def gaussian_profile(self, nt, width, t0=0.0):
    ...
    def scat_profile(self, nt, f, tau=1.0):
        """Include exponential scattering profile."""
        #        tau_nu = tau * (f / self._f_ref)**-4.
        tau_nu = tau * (f / 1000.0) ** -4.0
        t = np.linspace(0.0, nt // 2, nt)
    ...
    def pulse_profile(
        ...
        tdm_samp = tdm / tsamp
        dm_smear_prof = np.ones([max(1, np.int32(tdm_samp))])
    ...
    def add_to_data(self, delta_t, freq, data, scintillate=False, conv_dmsmear=False):
        ...
        if data.sum() == 0:
            # stds unused variable, commenting out
            # stds = 1
            pass
        else:
            # stds unused variable, commenting out
        ...
        for ii, f in enumerate(freq):
            ...
            if conv_dmsmear:
                ...
                width_ = self.calc_width(
                    0.0, f, bw=bandwidth, NFREQ=NFREQ, ti=self._width, tsamp=delta_t
                )
            else:
                width_ = self.calc_width(
                    self._dm,
...
def dm_transform_filobj(
    data, dt, freqs, ref_freq=np.inf, dm_min=-10, dm_max=10, ndm=50
):
    """Transform freq/time data to dm/time data."""
    if spectra is None:
        try:
            ...
            raise ImportError("spectra module not available")
...
class EventSimulator:
    ...
    def draw_event_parameters(self):
        ...
        fluence = uniform_range(*self._fluence) ** (-2 / 3.0)
        # Convert to Jy ms from Jy s
        fluence *= 1e3 * self._fluence[0] ** (-2 / 3.0)
    ...
    def draw_event_parameters_array(
        self,
        fluence_min=1,
        dm_min=10.0,
        dm_max=10000.0,
        ...
        dm[dm < dm_min] = dm_min
        dm[dm > dm_max] = dm_max
        fluence = fluence_min * np.random.uniform(0, 1, nfrb) ** (-2 / 3.0)
        spec_ind = np.random.uniform(spec_ind_min, spec_ind_max, nfrb)
        disp_ind = 2.0 * np.ones([nfrb])
        ...
        if fnout is not None:
            ...
            np.savetxt(
                ...
                params_arr,
                fmt="%.2f %.3f %.5f %.2f %.4f",
                header="DM  fluence  width  spec_ind  disp_ind",
            )
...
def gen_simulated_frb(
    ...
    # ES unused variable, commenting out
    # ES = EventSimulator(dm=dm, fluence=fluence, width=width, spec_ind=spec_ind)
    ...
    E = Event(t_ref, FREQ_REF, dm, fluence, width, spec_ind, disp_ind, scat_tau_ref)

    E.add_to_data(
        delta_t, freq, data, scintillate=scintillate, conv_dmsmear=conv_dmsmear
    )

    if plot_burst and plt is not None:
        plt.subplot(2, 1, 1)
        plt.imshow(
            data.reshape(-1, NTIME),
            aspect="auto",
        ...
        plt.subplot(3, 1, 3)
        plt.plot(data.reshape(-1, NTIME).mean(0))
